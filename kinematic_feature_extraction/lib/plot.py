...
def _generate_feat_map(gplot, subduction_data, projection, time, feature, output_dir):
    ...
    gplot.plot_plate_motion_vectors(ax, spacingX=10, spacingY=10, normalise=True, scale=50, alpha=0.1, zorder=3)
    ...
    gplot.plot_trenches(ax, color="black", alpha=0.3, zorder=6)
    gplot.plot_subduction_teeth(ax, spacing=0.015, color='black', alpha=0.3, zorder=7)
    ...
    cbar_feat = fig.colorbar(feat, shrink=0.4, pad=0.06, orientation="horizontal", extend="max")
